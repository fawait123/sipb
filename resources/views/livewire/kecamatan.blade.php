    ...
    <div class="card radius-10">
        <div class="card-body">
            <div class="d-flex align-items-center">
                <div>
                    <a href="{{ route('kecamatan.create') }}" class="btn btn-primary btn-sm">Tambah</a>
                </div>
                <div class="ms-auto">
                        ...
                        @if (count($query) > 0)
                            @foreach ($query as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->kabupaten->kabupaten }}</td>
                                    <td>{{ $row->kecamatan }}</td>
                                    <td>{{ $row->created_at->diffForHumans() }}</td>
                                    <td>
                                        <a href="{{ route('kecamatan.edit', $row->id) }}" class="text-primary"><i
                                                style="font-size: 19px" class="bx bx-message-square-edit"></i></a>
                                        <a href="{{ route('kecamatan.destroy', $row->id) }}"
